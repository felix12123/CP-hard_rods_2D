function A2_2()
	...
	Threads.@threads for i in eachindex(zs)
		...
		hist = histogram(obs[2][1:n_few], dpi=300, label="Nh", title="M=$M, L=$L, z=$(zs[i]), steps=$(n/100)", legend=:topleft, alpha=0.5, bins=100)
		histogram!(obs[3][1:n_few], label="Nv", alpha=0.5, bins=100)
		histogram!(twinx(), obs[1][1:n_few] ./ 2, label="N/2", alpha=0.5, bins=15, legend=:topright, color=:green)
